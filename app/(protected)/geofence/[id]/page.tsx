...
const extractSavedGeofenceId = (response: any, fallbackId = "0"): string => {
  const candidates = [
    response?.data?.zone?.id,
    response?.data?.geofence?.id,
    response?.data?.id,
    response?.data?.geoId,
    response?.id,
    response?.geoId,
...
export default function GeofenceDetailPage() {
  type MapToolMode = "draw" | "hand" | "circle" | "polygon";
  ...
  const [mapToolMode, setMapToolMode] = useState<MapToolMode>("circle");
  ...
  const getAccountNameById = useCallback(
    (value: number) =>
      accounts.find((account) => Number(account.id) === Number(value))?.value || "",
    [accounts],
  );
  ...
  useEffect(() => {
    const loadZone = async () => {
      if (isCreateMode) {
        ...
        const initialGeometry: GeometryType =
          geometryFromQuery === "polygon" ? "polygon" : "circle";
        setGeometry(initialGeometry);
        setMapToolMode(initialGeometry);
  ...
  const handleActivateDrawing = useCallback(
    (nextGeometry?: GeometryType, toolMode: MapToolMode = "draw") => {
      if (nextGeometry) {
        setGeometry(nextGeometry);
      }
      handleStartRedraw();
      setMapToolMode(toolMode);
    },
    [handleStartRedraw],
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response?.success) {
        const savedGeoId = extractSavedGeofenceId(response, id || "0");
        if (savedGeoId && savedGeoId !== "0") {
          setGeofenceSyncStatus(savedGeoId, {
            status: GEOFENCE_JAVA_SYNC_STATUS.UNSYNCED,
            message: "Pending Java sync",
          });
        }
        toast.success(isCreateMode ? t("toast.created") : t("toast.updated"));
  ...
  return (
    <div className={`${isDark ? "dark" : ""} mt-10`}>
      <ActionLoader
        isVisible={saving}
        text={isCreateMode ? "Creating geofence..." : "Updating geofence..."}
      />
      <div className={`min-h-screen ${isDark ? "bg-background" : ""} p-2`}>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
          <section className="xl:col-span-2 space-y-6">
            ...
            >
              <h3 className={`text-base font-semibold mb-4 ${isDark ? "text-gray-100" : "text-gray-900"}`}>
                {t("section.identity")}
              </h3>
            ...
            >
              <div className="mb-4">
                <h3 className={`text-base font-semibold ${isDark ? "text-gray-100" : "text-gray-900"}`}>
                  {t("fields.locationCenter")}
                </h3>
                <p className={`text-xs mt-1 ${isDark ? "text-gray-400" : "text-gray-500"}`}>
                  {getAccountNameById(accountId) || t("fields.selectAccount")}
                </p>
              </div>

              <div className={`relative rounded-2xl overflow-hidden border h-[520px] ${
                isDark ? "border-gray-800" : "border-gray-200"
              }`}>
                {isLoaded ? (
                  <>
                    <GeofenceMap
                      key={isRedrawing ? "drawing" : "viewing"}
